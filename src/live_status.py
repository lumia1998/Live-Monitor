...
class LiveStatusResolver:
    ...
    async def _check_source(self, source: RoomSource) -> LiveStatus:
        ...
        platform = "未知平台"
        try:
            record_quality = get_quality_code(source.quality)
            ...
            if "douyin.com/" in url:
                ...
                port_info = await stream.get_douyin_stream_url(json_data, record_quality, proxy_address)
                ...
                json_data = await spider.get_tiktok_stream_data(
                    url=url,
                    proxy_addr=proxy_address,
                    cookies=self.cookie("tiktok"),
                )
                port_info = await stream.get_tiktok_stream_url(json_data, record_quality, proxy_address)
                ...
                json_data = await spider.get_kuaishou_stream_data(
                    url=url,
                    proxy_addr=proxy_address,
                    cookies=self.cookie("kuaishou"),
                )
                port_info = await stream.get_kuaishou_stream_url(json_data, record_quality)
                ...
                if record_quality not in ["OD", "BD", "UHD"]:
                    json_data = await spider.get_huya_stream_data(
                        url=url,
                        proxy_addr=proxy_address,
                        cookies=self.cookie("huya"),
                    )
                    port_info = await stream.get_huya_stream_url(json_data, record_quality)
                else:
                ...
                platform = "小红书直播"
                port_info = await spider.get_xhs_stream_url(url, proxy_addr=proxy_address, cookies=self.cookie("xhs"))

            elif "www.bigo.tv/" in url or "slink.bigovideo.tv/" in url:
                platform = "Bigo直播"
                port_info = await spider.get_bigo_stream_url(url, proxy_addr=proxy_address, cookies=self.cookie("bigo"))
                ...
                port_info = await stream.get_stream_url(json_data, record_quality, spec=True)
                copy_extra(port_info, json_data, "new_cookies")

            elif "cc.163.com/" in url:
                platform = "网易CC直播"
                json_data = await spider.get_netease_stream_data(url=url, cookies=self.cookie("netease"))
                port_info = await stream.get_netease_stream_url(json_data, record_quality)
                ...
                port_info = await spider.get_qiandurebo_stream_data(
                    url=url,
                    proxy_addr=proxy_address,
                    cookies=self.cookie("qiandurebo"),
                )
                ...
                if isinstance(json_data, dict) and "play_url_list" in json_data:
                    port_info = await stream.get_stream_url(json_data, record_quality, spec=True)
                else:
                ...
                port_info = await stream.get_stream_url(json_data, record_quality, spec=False)
                ...
                json_data = await spider.get_baidu_stream_data(
                    url=url,
                    proxy_addr=proxy_address,
                    cookies=self.cookie("baidu"),
                )
                port_info = await stream.get_stream_url(json_data, record_quality)
                ...
                json_data = await spider.get_weibo_stream_data(
                    url=url,
                    proxy_addr=proxy_address,
                    cookies=self.cookie("weibo"),
                )
                port_info = await stream.get_stream_url(json_data, record_quality, hls_extra_key="m3u8_url")

            elif "kugou.com/" in url:
                platform = "酷狗直播"
                port_info = await spider.get_kugou_stream_url(url=url, proxy_addr=proxy_address, cookies=self.cookie("kugou"))
                ...
                json_data = await spider.get_acfun_stream_data(
                    url=url,
                    proxy_addr=proxy_address,
                    cookies=self.cookie("acfun"),
                )
                port_info = await stream.get_stream_url(json_data, record_quality, url_type="flv", flv_extra_key="url")
                ...
                platform = "京东直播"
                port_info = await spider.get_jd_stream_url(url=url, proxy_addr=proxy_address, cookies=self.cookie("jd"))
                ...
                port_info = await spider.get_picarto_stream_url(
                    url=url,
                    proxy_addr=proxy_address,
                    cookies=self.cookie("picarto"),
                )

            elif ".m3u8" in url or ".flv" in url:
                platform = "自定义直播源"
                port_info = {
                    "anchor_name": source.name or platform + "_" + str(uuid.uuid4())[:8],
                    "is_live": True,
                    "record_url": url,
                }
                if ".flv" in url:
    ...
    def status_from_port_info(
        ...
        stream_url = select_source_url(source.url, port_info) or ""
        return LiveStatus(
            id=source.id,
            url=source.url,
            ...
            quality=port_info.get("quality") or get_quality_code(source.quality),
            checked_at=checked_at,
            m3u8_url=port_info.get("m3u8_url") or "",
            flv_url=port_info.get("flv_url") or "",
    ...
    def status_from_error(self, source: RoomSource, platform: str, checked_at: str, error: str) -> LiveStatus:
        ...
        return LiveStatus(
            ...
            configured_name=source.name,
            quality=get_quality_code(source.quality),
            checked_at=checked_at,
            error=display_error,
        )
...
def get_quality_code(qn: str) -> str:
    return QUALITY_MAPPING.get(qn, "OD")
...
def select_source_url(link: str, stream_info: dict[str, Any]) -> str:
    if is_flv_preferred_platform(link):
        ...
        if flv_url:
            ...
            if codec and codec[0] == "h265":
                ...
                return flv_url
    return stream_info.get("record_url") or stream_info.get("m3u8_url") or stream_info.get("flv_url") or ""
